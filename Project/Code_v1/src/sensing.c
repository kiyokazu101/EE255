...
// will it help if IIR is here? maybe not 

  float  alpha = 0.2; // maybe this will help a bit?

  voltage_reading = alpha*last_voltage+(1.0-alpha)*(uint16_t) data[0];
  current_reading = alpha*last_current+(1.0-alpha)*(uint16_t) data[1];

  last_voltage = voltage_reading;
  last_current = current_reading;
